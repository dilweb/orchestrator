...
class OrderSagaRunner:
    ...
    async def handle_order_created(self, event: dict[str, Any]) -> None:
        ...
        logger.info("Saga started for order=%s", order.order_id)
    ...
    async def handle_inventory_reserved(self, event: dict[str, Any]) -> None:
        ...
        if order is None:
            logger.warning("Skip inventory.reserved for unknown order_id=%s", reserved.order_id)
            return
        ...
        logger.info("Inventory reserved for order=%s, payment charge initiated", order.order_id)
    ...
    async def handle_inventory_reserve_failed(self, event: dict[str, Any]) -> None:
        ...
        if order is None:
            logger.warning("Skip inventory.reserve-failed for unknown order_id=%s", failed.order_id)
            return
        ...
        logger.info("Order %s cancelled due to inventory failure", order.order_id)
    ...
    async def handle_payment_succeeded(self, event: dict[str, Any]) -> None:
        ...
        if order is None:
            logger.warning("Skip payment.succeeded for unknown order_id=%s", succeeded.order_id)
            return
    ...
    async def handle_payment_failed(self, event: dict[str, Any]) -> None:
        ...
        order = self._orders.pop(failed.order_id, None)
        if order is None:
            logger.warning("Skip payment.failed for unknown order_id=%s", failed.order_id)
            return
        ...
        logger.info("Order %s cancelled due to payment failure", order.order_id)
